...
We consider learning in MDPs, there exist two main trends for Safe RL. The first one is based on the modification of the optimality criterion to introduce the concept of risk. The second is based on the modification of the exploration process to avoid the exploratory actions that can lead the learning system to undesirable or catastrophic situations. As regard the first, there are several alternatives in order to consider risk:
\begin{itemize}
	\item Worst Case Criterion: a policy is considered to be optimal if it has the maximum worst-case return. This criterion is used to mitigate the effects of the variability induced by a given policy.
	\item Risk-Sensitive Criterion: the optimization criterion balances the return and the risk. A parameter allows the sensitivity to the risk to be controlled, risk can be defined as the variance of return or as the probability of entering into an error state.
	\item Constrained Criterion: it maximizes the return subject to one or more constraints.
\end{itemize}
As regard the modification of the exploration process, it can be modified through the incorporation of external knowledge. There are three ways of doing this:
\begin{itemize}
	\item Providing Initial Knowledge: examples gathered from a teacher or previous information on the task can be used to provide initial knowledge for the learning algorithm.
	\item Deriving a policy from a finite set of demonstrations: a set of examples provided by a teacher can be used to learn a model from which to derive a policy in an off-line and, hence, safe manner.
	\item Providing Teach Advice: a human or a simple controller assists the exploration during the learning process and provides advice.
\end{itemize}
