...
\end{align}
Starting from~\eqref{eq:pgt}, the performance gradient $\nabla_{\theta}J(\theta)$ can be estimated from samples in different ways. The estimated value is exactly the value used in~\eqref{eq:grad} to perform an update iteration of the policy parameter $\theta$.

\subsection{Policy Gradient Algorithms} \todo{QUESTA SEZIONE DEVE ESSERE SISTEMATA!!!}
