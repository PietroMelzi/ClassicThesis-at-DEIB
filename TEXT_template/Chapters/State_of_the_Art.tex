...
\begin{theorem}[Deterministic Policy Gradient Theorem] \label{th:dpgt}
	Suppose that $P(s'|s,a), \nabla_aP(s'|s,a), \mu_{\vtheta}(s), \nabla_{\vtheta} \mu_{\vtheta}(s), R(s,a), \nabla_a R(s,a), \mu(s)$ \hl{(the initial-state distribution)} \todo{mu usato per distribuzione iniziale degli stati e per policy deterministiche: correggere!!!!} are continuous in all parameters $\vtheta$ and variables $s, a, s'$. Then, 
	\begin{align}
