...
In this chapter, we test \ac{DPO} on simulated continuous control tasks. Details on task and policy definitions are provided for each experiment: in \mySec{sec:mini} for the experiment \emph{Minigolf}, in \mySec{sec:mass} for the experiment \emph{Double Integrator} and in \mySec{sec:safe} for the experiment \emph{Robot Adaptation}. In \mySec{sec:exp2} we provide details on how experiments were performed, we also show the results of the tuning phase performed on some hyperparameters. Additional results that help to understand better the behaviour of \ac{DPO} are provided in \mySec{sec:exp3}.

\section{Minigolf}\label{sec:mini}
We begin with a one-dimensional task, adapted from~\citet{doro2019gradient}, where the agent has to throw a ball into a hole by hitting it with a putter as few times as possible. The policy is an \ac{RBF} network mapping the scalar state (the ball-hole distance) to the scalar action (the applied force). 
...
Hyperparameters for all the algorithms are selected via grid search: regularization coefficient $\lambda$ and projection step size $\alpha$ for \ac{DPO}, initial policy standard deviation and learning rate for REINFORCE.
