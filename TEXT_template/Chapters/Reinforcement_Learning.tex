...
A \ac{MDP} is described by a six-tuple $M=\langle \Sspace, \Aspace, P, R, \gamma, p_0 \rangle$, where:
\begin{itemize}
...
As shown in [\cite{givan2000bounded}], $M_{\updownarrow}$ includes both an \ac{MDP} that simultaneously achieves $\underline{V}_{\pi}(s)$ for all $s \in \Sspace$ and another one that achieves $\overline{V}_{\pi}(s)$ for all $s \in \Sspace$.\\
\newline
...
\end{definition}
\noindent In the case of deterministic policies, the probability distributions $p$ and $q$ are Dirac delta functions. The Kantorovich distance between two Dirac delta functions is equal to the distance of their locations.\\
\newline
