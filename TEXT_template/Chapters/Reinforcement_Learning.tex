\chapter{Preliminaries for Reinforcement Learning} \label{chap:rl}
...
\end{align}
\hl{The intuitive explanation for} \myEq{eq:translation} \hl{is that the action prescribed by $\rho$ in the abstract state $X$ is copied into all the ground states $s \in X$. However, this translation does not ensure to preserve the performance of $\rho$ in $\wt{M}$ for $\pi$ in $M$.}\\
\newline
