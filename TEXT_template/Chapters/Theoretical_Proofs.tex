...
\begin{proof}
	...
	%
	&= Z_{\vtheta}(X)^{-1}\nonumber\\
	&\qquad \left|\int_{X}\delta^{\vtheta}(s')\left(Q^{\vtheta}(s,\pi_{\vtheta}(s)) - Q^{\vtheta}(s',\rho(X))\right)\de s'\right| \nonumber\\
	%
	&\leq Z_{\vtheta}(X)^{-1}\nonumber\\
	&\qquad \int_{X}\delta^{\vtheta}(s')\left|Q^{\vtheta}(s,\pi_{\vtheta}(s)) - Q^{\vtheta}(s',\rho(X))\right|\de s' \nonumber\\
	%
...
\begin{proof}
	...
	\begin{align}
	|W^{\rho}(X) - V^{\rho}(X)| 
	&= \left|\gamma \sum_{X'\in\Xspace} Z_{\vtheta}(X)^{-1}\right. \nonumber\\
	&\qquad\left. \int_{X}\delta^{\vtheta}(s)\int_{X'}p(s'|s,\rho(X))V^{\vtheta}(s')\de s'\de s 
	\right.\nonumber\\
	&\qquad\left. 
	- \gamma\sum_{X'\in\Xspace}\wt{P}_{\vtheta}(X'|X,\rho(X))V^{\rho}(X') \right| \label{p:13}\\
	%
	&= \gamma Z_{\vtheta}(X)^{-1} \nonumber \\
	&\qquad \left| \sum_{X'\in\Xspace} \int_{X}\delta^{\vtheta}(s)\int_{X'}p(s'|s,\rho(X))V^{\vtheta}(s')\de s'\de s 
	\right.\nonumber\\
	&\qquad\left. - \sum_{X'\in\Xspace}\int_{X}\delta^{\vtheta}(s)\int_{X'}p(s'|s,\rho(X))\de s'\de sV^{\rho}(X') \right| \label{p:14}\\
	%
	&\leq \gamma Z_{\vtheta}(X)^{-1} \sum_{X'\in\Xspace} \int_{X}\delta^{\vtheta}(s)\int_{X'}p(s'|s,\rho(X))\left|V^{\vtheta}(s') \right. \nonumber\\
	&\qquad \left. - V^{\rho}(X')\right|\de s'\de s \nonumber\\
	%
	&\leq \gamma Z_{\vtheta}(X)^{-1} \sum_{X'\in\Xspace} \int_{X}\delta^{\vtheta}(s)\int_{X'}p(s'|s,\rho(X))\left|V^{\vtheta}(s') \right. \nonumber\\
	& \qquad \left. - W^{\rho}(X')\right|\de s'\de s + 
	\gamma Z_{\vtheta}(X)^{-1} \sum_{X'\in\Xspace} \int_{X}\delta^{\vtheta}(s)\nonumber \\
	& \qquad \int_{X'}p(s'|s,\rho(X))\left|W^{\rho}(X') - V^{\rho}(X')\right|\de s'\de s \label{p:15}\\
	%
