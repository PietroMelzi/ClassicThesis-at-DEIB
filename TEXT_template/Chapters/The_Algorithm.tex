...
A fundamental aspect in the creation of the $\delta$-\ac{MDP} is the estimation of the $\gamma$-discounted future-state distribution $\delta^{\vtheta}(s)$\hl{, which we avoid to explicitly perform because it would require too many samples.} \todo{infinite?}
...
\end{align}
In the case of multiple optimal policies, we keep track of the entire set of optimal actions for each abstract state \hl{$X$ so that we can set, for every state $s \in X$, a possibly different optimal action of $X$ as target in the regression problem in} \mySubsec{subsec:proj}. \hl{By doing this, the difference between the deterministic policy before and after the update, considering only optimal actions in $\Xspace$, is minimized.}
...
\end{align}
\hl{where $\norm{A-\mathbb{I}}$ is the matrix norm of $A-\mathbb{I}$.}
